ea_spline2=function (obj, var.name, lam = 0.1, d = t(c(0, 5, 15, 30, 60, 
                                            100, 200)), vlow = 0, vhigh = 1000, show.progress = TRUE){
    ...
    for (st in 1:length(sp_dat)) {
      ...
      if (n == 1) {
        ...
        if(min(sp_dat[[st]][,2])!=0){
          yfit[,1:min(sp_dat[[st]][,2])]= NA
        }
